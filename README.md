<h1 align="center">Hi there 👋, I'm Forntoh Ngwing-allah Blandine</h1>
<h3 align="center">Cybersecurity Enthusiast | SOC & Blue Team | Penetration Testing & RedTeam</h3>

## 🔐 About Me

I am a final year student at The ICT University Cameroon, Major in Applied ICT in Cyber Security, focused on building real-world skills through practice.
Passionate about offensive and defensive cybersecurity

💡 Current Direction

-Security Operations (SOC)

-Vulnerability Assessment & Penetration Testing (VAPT)

🔍 Continuously expanding skills through platform-based labs (HackSmarter, HTB, THM) and real-world security implementations.

---

## 🧪 Projects

### 🔹 [SIEM Log Monitoring and Threat Detection Lab](https://github.com)
- **Goal:** Practice threat detection and incident response using a Windows 10 victim machine
- **Tools:** Splunk, Sysmon, windows 10, Linux
- **Outcome:** Used real-time logs to identify and correlate threat events following NIST IR standards.

### 🔹 [A Simple Python Port Scanner](https://github.com)
- **Goal:** Practice python scripting and learning how scanners operate
- **Tools:** Python
- **Outcome:** Used my python script to scan IP addresses to find open ports and banner information

---
  
## 📜 Certifications

| Issuer           | Certification | Year |
|------------------|----------------|------|
| Coursera/Learn2Quest   | [Learning Linux for LFCA](https://coursera.org/share/7265152f94e386253642bc42fc3fefec) | 2024 |

---

## ⚙️ Skills

### 🔧 Infrastructure & OS

| Category                | Tools Used |
|------------------------|------------|
| Virtualization         | ![VirtualBox](https://img.shields.io/badge/VirtualBox-4c4d51?style=flat&logo=virtualbox&logoColor=white) |
| Operating Systems      | ![Windows](https://img.shields.io/badge/Windows-0078D6?style=flat&logo=windows&logoColor=white) ![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat&logo=linux&logoColor=black) |

### 🌐 Networking & Monitoring

| Category                    | Tools Used |
|----------------------------|------------|
| Packet Analysis            | ![Wireshark](https://img.shields.io/badge/Wireshark-3A99FF?style=flat&logo=wireshark&logoColor=white) |

### 🛡️ Security Operations & Detection

| Category                      | Tools Used |
|------------------------------|------------|
| SIEM & Log Analysis          | ![Splunk](https://img.shields.io/badge/Splunk-0072C6?style=flat&logo=splunk&logoColor=white) |

### 🕷️ Ethical Hacking & Testing

| Category                          | Tools Used |
|----------------------------------|------------|
| Vulnerability Scanning           | ![Nmap](https://img.shields.io/badge/Nmap-00BFFF?style=flat&logo=nmap&logoColor=white) |
| Web App Testing             | ![Burp Suite](https://img.shields.io/badge/Burp_Suite-FF6600?style=flat&logo=burp-suite&logoColor=white) |
| Penetration Platforms            | ![Metasploit](https://img.shields.io/badge/Metasploit-000000?style=flat&logo=metasploit&logoColor=white) ![Kali Linux](https://img.shields.io/badge/Kali-557C94?style=flat&logo=kali-linux&logoColor=white) |

### 🛠️  Programming Languages
![Python](https://img.shields.io/badge/python-3670A0?style=for-the-badge&logo=python&logoColor=ffdd54) 
![Bash](https://img.shields.io/badge/bash-4EAA25.svg?style=for-the-badge&logo=gnu-bash&logoColor=white) 
![Java](https://img.shields.io/badge/java-%23ED8B00.svg?style=for-the-badge&logo=openjdk&logoColor=white) 

---
<h3 align="left">🌐 Connect with me:</h3>
<p align="left">
<a href="https://www.linkedin.com/in/blandine-forntoh-ngwing-allah-66172a24a/" target="_blank"><img align="center" src="https://raw.githubusercontent.com/rahuldkjain/github-profile-readme-generator/master/src/images/icons/Social/linked-in-alt.svg" alt="LinkedIn" height="30" width="40" /></a>
